...
本项目涵盖两类自动化路线：**COM 引擎驱动**（需要本机 Excel 进程）和**文件级操作**（无 Excel 进程）：

| 方式 | 接入方式 | 绑定 | 备注 |
|------|----------|------|------|
| **VBA** | 宿主内对象模型 | 早绑定 | Office 内嵌，"母语" |
| **C++** | `#import` + smart pointers | 早绑定 | 最底层、最可控 |
| **Rust** | `IDispatch` via `windows` crate | 晚绑定 | 零运行时、内存安全；当前参考实现 |
| **OpenXML (.NET)** | Open XML SDK，直接读写 `.xlsx` 文件 | 文件级 | 无需 Excel 进程；公式不重算；计划中 |

> VBA/C++ COM 实现完成了 E01–E12 标准任务集。Rust COM 是 capability 体系的参考后端（Cell I/O 域已验证）。
> OpenXML 后端尚未实现，将覆盖文件级读写场景。
...
spec/      标准任务集 E01–E12 + 功能覆盖矩阵 + capability 细粒度目录
...
除粗粒度 E01–E12 任务矩阵外，项目还维护一套**细粒度 capability 目录**，支持自动化
跨后端验证：

- [`spec/capabilities/`](spec/capabilities/) — capability 目录、contract、JSON schema、支持矩阵
- [`docs/backend-protocol.md`](docs/backend-protocol.md) — 新后端（OpenXML/C++/VBA）须实现的 JSON 协议

当前状态：**Rust 参考后端 + Cell I/O 域（9 个 capability）**；其余后端与域为计划中。
